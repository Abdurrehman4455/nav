...
const Nav = () => {
  return ( 
    ...
    <div className="w-[50%]  max-sm:w-[30%] flex items-center font-semibold pl-2 sm:pl-[10%] max-sm:h-[20px] max-sm:mt-2 max-sm:overflow=x-hidden">
        <img 
            src={Logo} 
            alt="logo" 
            className="w-[40px] max-sm:w-[60px]"
        />
        <span className="ml-2 font-bold text-lg sm:text-4xl leading-9 text-[#263238] max-sm:text-[20px]">Nexcent</span>
    </div>
        <div className="w-[50%] max-sm:w-[50%] flex  max-sm:flex-row max-sm:items-center mt-5 sm:mt-0 max-sm:overflow-hidden ">
        ...
        <div className="flex   max-sm:text-[14px] max-sm:text-center  max-sm:justify-cente">
            <Button label="Register Now" image={Right} className="mb-5 sm:mb-0" />
     </div>
